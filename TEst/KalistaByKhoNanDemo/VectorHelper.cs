...
namespace KalistaByKhoNanDemo.Kalista
{
    public class VectorHelper
    {
        ...
        {
            var objects = predefinedObjectList != null ? predefinedObjectList.ToList() : ObjectManager.Get<Obj_AI_Base>().Where(o => o.IsValidTarget(Player.Instance.GetAutoAttackRange(o))).ToList();
            var apexPoint = Player.Instance.ServerPosition.To2D() + (Player.Instance.ServerPosition.To2D() - Game.CursorPos.To2D()).Normalized() * Player.Instance.GetAutoAttackRange();

            return objects.Where(o => IsLyingInCone(o.ServerPosition.To2D(), apexPoint, Player.Instance.ServerPosition.To2D(), Math.PI)).OrderBy(o => o.Distance(apexPoint, true)).ToList();
        }
    }
}
