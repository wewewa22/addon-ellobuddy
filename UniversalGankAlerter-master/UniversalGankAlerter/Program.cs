...
namespace UniversalGankAlerter
{
    class Program
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            ...
            _menu = MainMenu.AddMenu("Cảnh Bao rừng Đồng Minh Gank", "Cảnh Bao rừng Đồng Minh Gank");
            _sliderRadius = new Slider("phạm vi kích hoạt", 3000, 500, 5000);
            _sliderRadius.OnValueChange += SliderRadiusValueChanged;
            _sliderCooldown = new Slider("Kích hoạt nguội (sec)", 10, 0, 60);
            _sliderLineDuration = new Slider("thời gian dòng (sec)", 10, 0, 20);
            _enemyJunglerOnly = new CheckBox("Cảnh báo jungler đối phương chỉ (đánh)", false);
            _allyJunglerOnly = new CheckBox("Cảnh báo đồng minh jungler chỉ (đánh)", true);
            _showChampionNames = new CheckBox("Hiển thị tên tướng", true);
            _drawMinimapLines = new CheckBox("Vẽ đường minimap", false);
            _dangerPing = new CheckBox("Nguy hiểm Ping (địa phương)", false);
            _enemies = _menu.AddSubMenu("Kẻ thù", "Kẻ thù");
            _enemies.Add<CheckBox>("chỉ kẻ thù jungler", _enemyJunglerOnly);

            _allies = _menu.AddSubMenu("đồng minh", "đồng minh");
            _allies.Add<CheckBox>("tất cả các jungler chỉ",_allyJunglerOnly);
            ...
            _menu.Add<CheckBox>("hiện tên", _showChampionNames);
            _menu.Add<CheckBox>("Vẽ đường vào minimap", _drawMinimapLines);
            _menu.Add<CheckBox>("nguy hiểm Ping", _dangerPing);
            ...
            foreach (AIHeroClient hero in ObjectManager.Get<AIHeroClient>())
            {
                if (hero.NetworkId != ObjectManager.Player.NetworkId)
                {
                    if (hero.IsEnemy)
                    {
                        ...
                        _enemies.Add<CheckBox>("kẻ thù" + hero.ChampionName,new CheckBox(hero.ChampionName));
                    }
                    else
                    {
                        _championInfoById[hero.NetworkId] = new ChampionInfo(hero, true);
                        _allies.Add<CheckBox>("đồng minh" + hero.ChampionName, new CheckBox(hero.ChampionName,false));
                    }
                }
            }
            //_menu.AddToMainMenu();
        ...
        {
            Chat.Print(
                "<font color='#ff3232'>thế giới</font><font color='#d4d4d4'>GankAlerter:</font> <font color='#FFFFFF'>" +
                msg + "</font>");
        }
        ...
        {
            return hero.IsEnemy
                ? _enemies["kẻ thù" + hero.ChampionName].Cast<CheckBox>().CurrentValue
                : _allies["đồng minh" + hero.ChampionName].Cast<CheckBox>().CurrentValue;
        }
    }
